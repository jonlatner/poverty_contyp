...
iv = "empst_1 + empst_3 + partner_1 + partner_3 + partner_4 + macro_unmp_rate + year"
...
saveRDS(df_output, file = paste0(results, "results_b_micro_macro.rds"))

df_output %>% filter(term=="macro_unmp_rate") %>% summarise(max = max(estimate), min = min(estimate))
