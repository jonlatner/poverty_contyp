...
world_bank = "support_files/world_bank/"
...
df_eu_silc <- readRDS(file = paste0(data_files,"02_df_eu_silc_trt_sample_4year.rds")) %>%
        arrange(country,panel,pid,year) %>%
        select(country,panel,pid,year,everything())
        

# Merge cpi measure from eurostat -----------------------------------------

# https://ec.europa.eu/eurostat/web/hicp/data/database
# HICP - annual data (average index and rate of change) (prc_hicp_aind) 	
df_cpi <- read.table(file = paste0(support_files,"prc_hicp_aind.tsv"), sep = '\t', header = TRUE) 

df_cpi <- df_cpi %>% 
        separate(unit.coicop.geo.time, c("unit", "coicop", "geo"), sep = ",") %>%
        filter(coicop=="CP00")
df_cpi <- df_cpi %>%
        ...
        rename(country=geo) %>%
        mutate(country = ifelse(country == "EL", yes = "GR", no = country)) %>% # rename country code for Greece
        select(-unit,-coicop)

df_cpi <- df_cpi %>% 
        separate(cpi, c("cpi", "note"), sep = " ") %>%
        select(-note)

df_cpi$cpi <- gsub(" ", "", df_cpi$cpi, fixed = TRUE)
df_cpi$cpi <- as.numeric(as.character(df_cpi$cpi))
df_cpi$year <- as.numeric(as.character(df_cpi$year))

df_eu_silc <- merge(df_eu_silc, 
                      df_cpi, 
                      by = c("country","year"), 
                      all.x = TRUE)

# create a measure for absolute poverty level in the first year of a panel wave
df_eu_silc <- df_eu_silc %>%
        arrange(country,panel,year) %>%
        group_by(country,panel) %>%
        mutate(poverty_line = median_inc*.6,
               poverty_line_rel = first(median_inc*.6), 
        ) %>%
        ungroup() %>%
        mutate(eq_hh_income_adj=eq_hh_income*(cpi/100), # inflation adjust household income
               poverty_line_abs=poverty_line_rel*(cpi/100), # inflation adjust poverty line
               pov_2 = ifelse(eq_hh_income_adj<poverty_line_abs, yes = 1, no = 0)) %>%
        arrange(country,panel,pid,year)
        
df_eu_silc <- df_eu_silc %>%
        select(-pov,-eq_hh_income,-median_inc,-matches("poverty_line"),-cpi) %>%
        mutate(pov=pov_2)
        
# Load macro variables from eurostat -----------------------------------------

# Unemployment - from the world bank
df_unmp <- read.csv(paste0(world_bank,"world_bank_unemployment.csv"))
...
df_eu_silc <- df_eu_silc %>%
        ...
        select(country, country_name, panel, pid, id, hid, year, male, age, pov, matches("exit_pov"), empst, ftc, ftime, empst_v1, empst_v2, partner_v1, matches("macro"), weight_long, kids_in_hh, num_kids) %>%
...
df_eu_silc <- df_eu_silc %>%
        group_by(country,panel,pid) %>%
        mutate(weight_long=last(weight_long)) %>%
        ungroup()

# Save ----

saveRDS(df_eu_silc, file = paste0(data_files, "03_df_eu_silc_trt_sample_clean_poverty_line_anchor.rds"))
