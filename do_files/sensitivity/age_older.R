...
df_eu_silc <- df_eu_silc %>%
        mutate(age_cat = ifelse(age < 35, yes = 1,
                                ifelse(age > 45, yes = 3, no = 2)))

t <- table(df_eu_silc$age_cat)
prop.table(t)
...
ggsave(plot = p, filename = paste0(graphs,"graph_sensitivity_micro_macro_int_older.pdf"), height = 9, width = 10.5)
