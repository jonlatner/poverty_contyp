...
df_eu_silc <- readRDS(paste0(data_files,"03_df_eu_silc_trt_sample_clean_2year.rds"))
...
df_output <- df_mfx_exit_t %>%
        mutate(country_name_region = country)

rm(df_mfx_exit_t)

saveRDS(df_output, file = paste0(results, "results_c_micro_macro_int_2year.rds"))
