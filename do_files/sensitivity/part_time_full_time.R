...
df_pov_exit <- df_pov_exit %>%
        ...
        select(region,country_name_region,id,year,matches("exit_pov_"),matches("empst_v2"),matches("partner_"),matches("unmp"))

# empst_v2: 
# 1=unemployed, 
# 2=employed (temp + ptime), 
# 3=employed (temp + ftime), 
# 4=employed (perm + ptime), 
# 5=employed (perm + ftime)

df_pov_exit <- df_pov_exit %>%
        mutate(empst_1 = ifelse(empst_v2==1, yes = 1, no = 0), #unemployed
               empst_2 = ifelse(empst_v2==2, yes = 1, no = 0), #employed, with a temporary contract + part time
               empst_3 = ifelse(empst_v2==3, yes = 1, no = 0), #employed, with a temporary contract + full time
               empst_4 = ifelse(empst_v2==4, yes = 1, no = 0), #employed, with a permanent contract + part time
               empst_5 = ifelse(empst_v2==5, yes = 1, no = 0), #employed, with a permanent contract + part time
...
df_pov_exit <- df_pov_exit %>%
        mutate(intunmp_empst_1 = macro_unmp_rate*empst_1,
               ...
               intunmp_empst_4 = macro_unmp_rate*empst_4,
               intunmp_empst_5 = macro_unmp_rate*empst_5
        )
...
iv = "empst_1 + empst_2 + empst_4 + empst_5 + 
partner_1 + partner_3 + partner_4 +
macro_unmp_rate + 
intunmp_empst_1 + intunmp_empst_2 + intunmp_empst_4 + intunmp_empst_5 +
year"
...
df_graph$term <- factor(df_graph$term,
                        levels = c(
                                ...
                                "intunmp_empst_1", "intunmp_empst_2", "intunmp_empst_4", "intunmp_empst_5",  
                                "partner_1", "partner_3", "partner_4",
                                "empst_5", "empst_4", "empst_2", "empst_1"
                        ),
                        labels = c(
                                "Unmp rate (%)",
                                "Unemployed", "Temp+ftime", "Perm+ptime", "Perm+ftime",  
                                "Single", "Employed spouse", "Children", 
                                "Perm+ftime", "Perm+ptime", "Temp+ftime", "Unemployed"
                        ))
...
table(df_graph$term)
with(df_graph,table(term,group))
...
p <- ggplot(df_graph, aes(x = country, y = estimate, color = sig, shape = sig)) +
        ...
        geom_text(aes(label=ifelse(p.value<.05,sprintf(estimate, fmt = '%#.3f'),"")),
                  hjust=ifelse(df_graph$estimate<0, yes = +1.1, no = -.1),
                  show.legend = FALSE,
                  size=3) +
...
ggsave(plot = p, filename = paste0(graphs,"graph_sensitivity_micro_macro_int_ptime_ftime.pdf"), height = 9, width = 10.5)
