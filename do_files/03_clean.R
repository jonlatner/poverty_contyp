...
# Head education status ----

df_eu_silc$isced_cat_1 <- recode(df_eu_silc$isced, "0:2=1; 3=2; 4=3; 5:6=4;
                                 34=3; 35=3; 
                                 100:200=1; 300:354=2; 400:499=3; 500:900=4")

# 1 - less than secondary, 2 - secondary, 3 - more than secondary
df_eu_silc$edu_cat <- recode(df_eu_silc$isced_cat_1, "1=1; 2=2; 3:4=3")

# Clean education

df_eu_silc <- df_eu_silc %>%
        group_by(panel, country, pid) %>%
        mutate(edu_cat = na.locf(edu_cat, na.rm=FALSE),
...
df_eu_silc <- df_eu_silc %>%
        ...
        select(country, country_name, panel, pid, id, hid, year, male, age, pov, matches("exit_pov"), empst, ftc, ftime, empst_v1, empst_v2, partner_v1, matches("macro"), weight_long, kids_in_hh, num_kids, edu_cat) %>%
...
saveRDS(df_eu_silc, file = paste0(data_files, "03_df_eu_silc_trt_sample_clean_4year.rds"))
